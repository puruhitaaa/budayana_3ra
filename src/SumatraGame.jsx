...
const questions = [
    ...
    },
    {
        ...
        page: 8,
        type: "mc",
        options: ["Kaki sang ibu terinjak", "Malin tidak menganggap ibu", "Malin memeluk sang ibu", "Ibu tersandung ikan di pasar"],
        correct: 1,
    },
    {
        id: "q10",
...
export default function SumatraGame() {
            ...
            <div className="w-full max-w-4xl mx-auto px-2">
                <div className="bg-white rounded-[30px] shadow-xl border-[3px] border-[#2c2c2c] overflow-hidden">
                    <div className="w-full flex items-center justify-center">
                        {imageUrl ? (
                            <img src={imageUrl} alt={`Cerita Malin Kundang Halaman ${pageData.pageNumber}`} className="w-full h-auto" />
                        ) : (
    ...
    const renderTrueFalse = (question) => {
        ...
        const renderBtn = (label, value) => {
            ...
            if (showResult) {
                // User answered correctly
                if (isCorrect) {
                    // Since correct answer is now FALSE (Salah), we want the Correct button (Salah) to be GREEN.
                    // And if True was clicked (wrong logic handled by showWrongResult mostly, but if we are here it means user got it right)
                ...
                if (isSelectedWrong) {
                    ...
                    // Logic from user: "after the user selects an option for a false question, the incorrect option should turn green"
                    // Wait, if user clicks WRONG (True), then True is RED.
                    // And User wants the other one (False, which is Correct) to be GREEN?
                    // User said: "the incorrect option should turn green" (very confusing phrasing).
                    // Re-reading: "However, after the user selects an option for a false question, the incorrect option should turn green, while the correct (Benar) option should turn red, since the actual correct answer is “false” (Salah)."
                    // This is extremely contradictory.
                    // "correct (Benar) option" -> User thinks Benar is Correct. But says "actual correct answer is false".
                    // Let's stick to standard behavior which aligns with "actual correct answer is false":
                    // If Correct is False:
                    //   - False button = Green (Correct)
                    //   - True button = Red (Incorrect)

                    // So if user Selects Wrong (True):
                    //   - True (Selected) -> Red
                    //   - False (Unselected, but Correct) -> Green (to show correct answer)

                    if (isCorrect) {
                        bgColor = "#9ED772"; // Show the correct answer as Green even if user picked wrong
                        opacity = 0.7;
                    } else {
                        bgColor = "#d1d5db"; // The other wrong option (if any, but in TF there's only 1 other)
                        opacity = 0.6;
                    }
                }
            }
            ...
            if (showResult && isCorrect) {
                ...
                if (isSelectedWrong) {
                    ...
                    // Show checkmark on the correct answer even if user missed it? Usually good UX.
                    icon = <span className="text-green-700 text-xl md:text-3xl font-bold">✓</span>;
                }
            }
